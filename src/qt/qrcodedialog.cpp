...
{
    QString ret = QString("wyvern:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WYV unit input in lnReqAmount, we generate the URI with WYV as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(wyvernUnits::format(wyvernUnits::WYV, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
