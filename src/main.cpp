...
const string strMessageMagic = "wyvern very Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // wyvern: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // wyvern: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    // Store transaction in memory
    {
        ...
        try {
          addAddressIndex(tx, mapInputs);
        } catch(const std::exception& e) {
          printf("CTxMemPool::accept() : unable to add tx %s to address index, probably missing inputs (reorg?)\n", tx.GetHash().ToString().c_str());
        }
        try {
          addSpentIndex(tx, mapInputs);
        } catch(const std::exception& e) {
          printf("CTxMemPool::accept() : unable to add tx %s to spent index, probably missing inputs (reorg?)\n", tx.GetHash().ToString().c_str());
        }
    }
...
// wyvern: find block wanted by given orphan block
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight < 100) { nSubsidy = 0 * COIN; }
    if (pindexBest->nHeight >= 100) { nSubsidy = 50 * COIN; }
    if (pindexBest->nHeight >= 25300) { nSubsidy = 25 * COIN; }
    if (pindexBest->nHeight >= 50500) { nSubsidy = 15 * COIN; }
    if (pindexBest->nHeight >= 75700) { nSubsidy = 10 * COIN; }
    if (pindexBest->nHeight >= 100900) { nSubsidy = 5 * COIN; }
...
// wyvern: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // wyvern: target change every block
    // wyvern: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // fMiner is true when called from the internal wyvern miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // wyvern: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    if (!txdb.UpdateAddressUnspentIndex(addressUnspentIndex)) {
        return error("DisconnectBlock() : Failed to update unspent index");
    }
    ...
    // wyvern: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx) {
        SyncWithWallets(tx, this, false, false);
        SyncWithWalletsV(tx, this);
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // wyvern: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // wyvern: track money supply and mint amount info
    ...
    if (addressIndex.size() > 0) {
      ...
      if (!txdb.UpdateSpentIndex(spentIndex))
...
// wyvern: total coin age spent in transaction, in the unit of coin-days.
...
// wyvern: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // wyvern: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // wyvern: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // wyvern: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // Wyvern: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // wyvern: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // wyvern: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // wyvern: getblocks may not obtain the ancestor block rejected
    ...
    // wyvern: if responsible for sync-checkpoint send it
...
// wyvern: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "wyvern", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Wyvern";
        CTransaction txNew;
        txNew.nTime = 1490705190;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xb3ae5dc25a7f39a9ea08cdc8b349554384dbc1825ef959ca25a766d19cd556e6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb5, 0xb6, 0xb7, 0xd5 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // wyvern: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // wyvern: tell downloading node about the latest block if it's
...
bool GetAddressIndex(uint160 addressHash, int type, std::vector<std::pair<CAddressIndexKey, int64_t> > &addressIndex, int start, int end)
{
    CTxDB txdb("r");
...
bool GetTimestampIndex(const unsigned int &high, const unsigned int &low, std::vector<uint256> &hashes)
{
    ...
    if (!txdb.ReadTimestampIndex(high, low, hashes))
...
void CTxMemPool::addAddressIndex(const CTransaction& tx, const MapPrevTx& mapInputs)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            ...
            CMempoolAddressDelta delta(time, prevout.nValue * -1, input.prevout.hash, input.prevout.n);
    ...
    for (unsigned int k = 0; k < tx.vout.size(); k++) {
        ...
        if (out.scriptPubKey.IsPayToScriptHash()) {
            ...
            mapAddress.insert(make_pair(key, CMempoolAddressDelta(time, out.nValue)));
            inserted.push_back(key);
...
void CTxMemPool::addSpentIndex(const CTransaction& tx, const MapPrevTx& mapInputs)
{
    LOCK(cs);
...
bool GetSpentIndex(CSpentIndexKey &key, CSpentIndexValue &value)
{
    ...
    if (!txdb.ReadSpentIndex(key, value))
