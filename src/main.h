...
#ifndef WYVERN_MAIN_H
#define WYVERN_MAIN_H
...
static const uint256 hashGenesisBlock("0x00000d2d49e496775915855444e6299109e9d881b7e1253577ae110d3e0e73de");
static const uint256 hashGenesisBlockTestNet("0x00003207d4daa5f3ae62d42abbd0387a9f6ca4f881ac2f0f1ae2c335058af8ef");
...
struct CAddressIndexKey {
    ...
    CAddressIndexKey(unsigned int addressType, uint160 addressHash, int height, int blockindex, uint256 txid, size_t txindex, bool isSpending) {
        type = addressType;
...
public:
    ...
    {
        // wyvern: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    /** Amount of wyverns spent by this transaction.
    ...
    /** Amount of wyverns coming in to this transaction
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // wyvern: get transaction coin age
...
public:
    ...
    // wyvern: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // wyvern: two types of block: proof-of-work or proof-of-stake
    ...
    // wyvern: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // wyvern: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // wyvern: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // wyvern: block index flags
    enum
    {
...
public:
    ...
    void addAddressIndex(const CTransaction &tx, const MapPrevTx& mapInputs);
    bool getAddressIndex(std::vector<std::pair<uint160, int> > &addresses,
                     std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results);
    bool removeAddressIndex(const uint256 txhash);
    void addSpentIndex(const CTransaction &tx, const MapPrevTx& mapInputs);
    bool getSpentIndex(CSpentIndexKey &key, CSpentIndexValue &value);
